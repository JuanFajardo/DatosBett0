...
<table id="tabla" class="table" width="100%">
  <thead>
    <tr>
        <th> Exp. </th> <th> Nombre Y Apellidos </th> <th> Sexo </th> <th> CI</th> <th> Ext.</th> <th> E. Civil </th> <th> Telefono</th> <th> Zona / Direccion</th> <th> Dep. / Pro.</th> <th> Acciones </th>
    </tr>
  </thead>
  <tbody>
      ...
      <tr data-id="{{ $dato->id }}">
          ...
          <td>
            @if($dato->genero == 'FEMENINO')
              <i class="badge badge-info"> {{$dato->nombres}}, {{$dato->apellido_paterno}} {{$dato->apellido_materno}} </i>
            @else
              <i class="badge badge-warning"> {{$dato->nombres}}, {{$dato->apellido_paterno}} {{$dato->apellido_materno}} </i>
            @endif
          </td>
          <td> @if($dato->genero == 'FEMENINO') <i class="badge badge-info">{{$dato->genero}}</i> @else <i class="badge badge-warning">{{$dato->genero}}</i> @endif</td>
          <td>{{$dato->ci}}</td>
          <td>{{$dato->extendido}}</td>
          <td>{{$dato->estado_civil}}</td>
          <td>{{$dato->telefono}}</td>

          <td><b>{{$dato->zona}}</b> | {{$dato->domicilio}}</td>
          <td><b>{{$dato->departamento}}</b> | {{$dato->provincia}}</td>
